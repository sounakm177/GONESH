...
async function generateNew(domain = null) {
  ...
  try {
    ...
    // Switch Reverb subscription to new mailbox channel
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function subscribeToMailbox(mailboxId) {
        ...
        if (currentChannel) {
            ...
            console.log('[Reverb] 🔄 Left channel:', currentChannel);
        }

        currentChannel = channelName;

        window.Echo.channel(channelName)
            .listen('.new.email', (data) => {
                console.log('[Reverb] 📩 New Email:', data);
                onNewEmail(data);
            })
            .listen('.mailbox.expired', () => {
                console.warn('[Reverb] ⏳ Mailbox expired');
                handleExpiry();
            });
